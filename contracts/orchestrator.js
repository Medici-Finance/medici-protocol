...
async function main() {
    ...
    let network = config.local[process.argv[2]];
    if (!network) {
    ...
    if (process.argv[3] == "deploy") {
        if (network.type == "evm") {
            ...
            exec(
                // `cd src/core && forge build && forge create --legacy
                // --rpc-url ${network.rpc} --private-key ${network.privateKey}
                // MediciCore.sol:MediciCore && exit`
                `cd src/core && forge build && forge create --legacy`,
                (err, out, errStr) => {
                    if (err) {
                        throw new Error(err);
